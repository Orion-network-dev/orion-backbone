...
var (
	...
	routerId         = flag.Uint("router-id", 1, "Specify the router id, this is used for multihoming operation")
...
func (c *OrionClientDaemon) resolveIdentity() error {
	...
	if *memberIdOverride != 0 {
		c.identity = &proto.Router{
			MemberId: uint32(*memberIdOverride),
		...
		if len(matches) == 2 {
			...
			c.identity.MemberId = uint32(number)
		} else {
	...
	c.identity.RouterId = uint32(*routerId)
	...
	if *asn != 0 {
		...
		c.asn = 64511 + c.identity.MemberId
	}
